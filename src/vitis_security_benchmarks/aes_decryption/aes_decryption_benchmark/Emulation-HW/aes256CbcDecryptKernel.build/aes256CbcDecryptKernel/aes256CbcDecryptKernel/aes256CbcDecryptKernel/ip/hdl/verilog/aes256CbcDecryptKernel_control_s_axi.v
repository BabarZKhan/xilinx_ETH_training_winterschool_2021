...
// ==============================================================
`timescale 1ns/1ps
module aes256CbcDecryptKernel_control_s_axi
#(parameter
...
localparam
    ...
    ADDR_IER               = 6'h08,
    ADDR_ISR               = 6'h0c,
    ADDR_INPUTDATA_DATA_0  = 6'h10,
    ADDR_INPUTDATA_DATA_1  = 6'h14,
    ADDR_INPUTDATA_CTRL    = 6'h18,
    ADDR_OUTPUTDATA_DATA_0 = 6'h1c,
    ADDR_OUTPUTDATA_DATA_1 = 6'h20,
    ADDR_OUTPUTDATA_CTRL   = 6'h24,
    WRIDLE                 = 2'd0,
    WRDATA                 = 2'd1,
    WRRESP                 = 2'd2,
    WRRESET                = 2'd3,
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_OUTPUTDATA_DATA_0: begin
                    rdata <= int_outputData[31:0];
                end
                ADDR_OUTPUTDATA_DATA_1: begin
                    rdata <= int_outputData[63:32];
                end
            endcase
        end
    end
...
assign outputData  = int_outputData;
// int_event_start
always @(posedge ACLK) begin
    if (ARESET)
...
always @(posedge ACLK) begin
    if (ARESET)
        int_outputData[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_OUTPUTDATA_DATA_0)
            int_outputData[31:0] <= (WDATA[31:0] & wmask) | (int_outputData[31:0] & ~wmask);
    end
end

// int_outputData[63:32]
always @(posedge ACLK) begin
    if (ARESET)
        int_outputData[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_OUTPUTDATA_DATA_1)
            int_outputData[63:32] <= (WDATA[31:0] & wmask) | (int_outputData[63:32] & ~wmask);
    end
end
