...
entity aes256CbcDecryptKernel_control_s_axi is
generic (
...
end entity aes256CbcDecryptKernel_control_s_axi;
...
architecture behave of aes256CbcDecryptKernel_control_s_axi is
    ...
    constant ADDR_INPUTDATA_DATA_0  : INTEGER := 16#10#;
    constant ADDR_INPUTDATA_DATA_1  : INTEGER := 16#14#;
    constant ADDR_INPUTDATA_CTRL    : INTEGER := 16#18#;
    constant ADDR_OUTPUTDATA_DATA_0 : INTEGER := 16#1c#;
    constant ADDR_OUTPUTDATA_DATA_1 : INTEGER := 16#20#;
    constant ADDR_OUTPUTDATA_CTRL   : INTEGER := 16#24#;
    ...
    signal int_outputData      : UNSIGNED(63 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_OUTPUTDATA_DATA_0 =>
                        rdata_data <= RESIZE(int_outputData(31 downto 0), 32);
                    when ADDR_OUTPUTDATA_DATA_1 =>
                        rdata_data <= RESIZE(int_outputData(63 downto 32), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    outputData           <= STD_LOGIC_VECTOR(int_outputData);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_OUTPUTDATA_DATA_0) then
                    int_outputData(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_outputData(31 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_OUTPUTDATA_DATA_1) then
                    int_outputData(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_outputData(63 downto 32));
                end if;
            end if;
        end if;
    end process;
